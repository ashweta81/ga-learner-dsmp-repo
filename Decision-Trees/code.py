...
plt.bar(x = fully_paid.index, height = fully_paid.values)
plt.show()
# Code ends here
...
# Code starts here
X_train['int.rate'] = X_train['int.rate'].apply(lambda x : x.strip('%')).astype(float)
X_train['int.rate'] = X_train['int.rate']/100

X_test['int.rate'] = X_test['int.rate'].apply(lambda x : x.strip('%')).astype(float)
